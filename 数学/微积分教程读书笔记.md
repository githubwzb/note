...
## 第二章 一元函数
### 1. 极限定理的推广
若当$x$趋于$a$时，函数$f(x)$也有一有限的极限$A$，而$A>p(A<q)$，则当$x$的值充分接近于$a$（异于$a$）时，函数本身也就满足不等式
$$
f(x)>p (f(x)<q)
$$
选取正数$\epsilon<A-p(q-A)$，就有
$$
A-\epsilon>p(A+\epsilon<q)
$$

依据极限的定义，对于这$\epsilon$必能求出$\delta$只需$|x-a|<\delta$（其中$x$取自$X$且异于$a$
